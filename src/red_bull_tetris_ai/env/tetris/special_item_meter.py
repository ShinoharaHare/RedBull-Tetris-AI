...
class SpecialItemMeter(pygame.sprite.Sprite):
    def __init__(
        self,
        size: tuple[int, int],
    ...
    def update(self, state: TetrisState) -> None:
        ...
        text = self.font.render('SP', True, (255, 255, 255))
        ...
        bar_rect = pygame.Rect(
            outline_rect.left,
            outline_rect.top + outline_rect.height * (1 - state.special_item_meter),
            outline_rect.width,
            int(outline_rect.height * state.special_item_meter)
        )
